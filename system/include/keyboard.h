...
// Flushes any pending scancodes
void keyboard_flush_buffer(void);

// Alternate top level interface that reads a char and also returns
// the active modifiers at the time the char was read
unsigned char keyboard_read_mod_char(keyboard_modifiers_t *mods);
