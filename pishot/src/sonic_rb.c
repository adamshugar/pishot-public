 ...
 *
 * Adapted for use in HC-SR04 asynchronous driver by Adam Shugar
 * on March 11, 2020.
 */
...
{
    sonic_rb_t *rb = malloc(sizeof(struct sonic_rb));
    rb->head = rb->tail = 0;
    return rb;
}
...
{
    if (sonic_rb_full(rb)) return false;
...
{
    if (sonic_rb_empty(rb)) return false;
